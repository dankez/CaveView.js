...
import { ParsedCave } from '../parsers/caveParser';
...
export const CalibrationModal: React.FC<Props> = ({ svgText, cave, onCalibrate, onClose }) => {
  ...
  const stationNames = Array.from(new Set(cave.stationLabels.map(l => l.name))).sort();
  ...
  const submit = () => {
    ...
    const matches = points.map(p => {
      const caveS = cave.stationLabels.find(l => l.name === p.stationName);
