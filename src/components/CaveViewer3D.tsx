  ...
  useEffect(() => {
    ...
    if (isWmsXyz && surface.sjtskBbox) {
      ...
      if (source === 'wms-geology') {
        const resolution = props.options.surfaceWmsResolution || 2048;
        downloadPromise = downloadWmsImage(url, surface.sjtskBbox, resolution, resolution, format, (p) => {
          if (!isActive) return;
          const progress = Math.round((p.current / p.total) * 100);
          setWmsProgress(progress);
          onStatusChange?.({ msg: 'Sťahujem mapové podklady (ŠGÚDŠ)...', type: 'progress', progress });
        });
      } else {
        ...
        downloadPromise = downloadTiledXyz(url, surface.sjtskBbox, format, (p) => {
          ...
          const progress = Math.round((p.current / p.total) * 100);
          setWmsProgress(progress);
          onStatusChange?.({ msg: `Sťahujem mapové podklady (${source.includes('freemap') ? 'Freemap' : 'ZBGIS'})...`, type: 'progress', progress });
        }, zoomLevel);
      }
